...
import { useSabreStatus, useUpdateCredentials, useUpdateEnterprise } from '@/hooks/useEnterprises'
...
export function SettingsCard({ ent }: { ent: Enterprise }) {
  ...
  const dirty = name !== (ent.name ?? '') || search !== ent.searchApiVersion || booking !== ent.bookingApiVersion
...
export function SabreCard({ ent }: { ent: Enterprise }) {
...
export function ZentrumhubCard({ ent }: { ent: Enterprise }) {
...
export function SabreStatusCard({ id }: { id: string }) {
